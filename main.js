...
const pools = [
  ...
  // OmanNum  <- total - AmanNum - BmanNum - CmanNum}
];
...
const findStepIndexForStartNewSchedule = () => {
  return 0;
};
 ...
 */
const findPoolByTagetSchedule = (targetDay, source) => {
  ...
  if (!targetPool) {
    targetPool = {
      ...
      bmanNum: Total * weights.Bman,
      cmanNum: Total * weights.Cman,
      omanNum: Total * (1 - weights.Aman - weights.Bman - weights.Cman),
    };
...
const scheduler = (howDo /** add, remove, continue */, waitHandleIds) => {
  if (howDo === "continue") {
    ...
    for (const data of waitHandleIds) {
      ...
      const newScheduleStartDay = calcScheduleStartDay(data.startDay, data.type);
      // find target pool in next schedule
      const targetPool = findPoolByTagetSchedule(
        newScheduleStartDay,
        data.source
      );
      // record
  ...
  if (howDo === "add") {
    ...
    for (const item of waitHandleIds) {
      ...
      //   block = 0;
      const targetPool = findPoolByTagetSchedule(startDay, item.source);
